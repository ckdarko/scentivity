...
function compactProductCard(product = {}, options = {}) {
  ...
  return `
    <article class="product-card compact-product-card product-link-card" data-product-url="${productPageUrl(key)}" tabindex="0" aria-label="View details for ${name}">
      <div class="compact-product-image">
        ...
        <span class="compact-product-badge">Available</span>
      </div>
...
function renderHomepageShowcase() {
    ...
    homepageProductSlides.innerHTML = `
      <article class="showcase-slide active fallback-showcase">
        ...
        <div class="showcase-copy">
          ...
          <p>Available products will appear here when added in the admin page.</p>
        </div>
      </article>
    `;
  ...
  homepageProductSlides.innerHTML = `
    <article class="showcase-slide active compact-showcase-slide product-link-card" aria-label="${name}" data-product-url="${productPageUrl(key)}" tabindex="0">
      <div class="showcase-image-wrap">
        <img src="${image}" alt="${name}" loading="lazy" onerror="this.onerror=null;this.src='assets/scentivity-logo-fused.png';" />
        <span class="showcase-badge available">Available now</span>
      </div>
      <div class="showcase-copy">
        <p class="eyebrow">Available now • ${showcaseIndex + 1} of ${slides.length}</p>
        <h3>${name}</h3>
