...
// Product card/detail click behavior: card opens details; only cart icon/button adds to cart.
document.addEventListener('click', event => {
  ...
  const closeDetail = event.target.closest('#closeProductDetail, .product-detail-close');
  if (closeDetail || event.target.id === 'productDetailModal') {
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
    closeProductDetail();
  ...
  if (productTrigger && !event.target.closest('button.add-to-cart, [data-deal-product-key], a, input, select, textarea, label')) {
    ...
    event.stopPropagation();
    event.stopImmediatePropagation();
    openProductDetail(productTrigger.dataset.productKey);
    return;
  }
}, true);
