// SCENTIVITY_CLICKABLE_PRODUCT_DETAILS_20260608
...
const closeProductDetailModal = document.querySelector('#closeProductDetailModal');
...
function getProductDetailText(product = {}) {
  return cleanText(product.productDetails || product.description || product.details || product.notes || 'Sweet, elegant scent selected by Scentivity for everyday freshness, gifting, and memorable moments.');
}

function getProductFragranceNotes(product = {}) {
  return cleanText(product.fragranceNotes || product.scentNotes || product.notes || 'Add fragrance notes in the admin dashboard.');
}

function getProductIngredients(product = {}) {
  return cleanText(product.ingredients || 'Ingredients information can be added in the admin dashboard.');
}

function getProductReturnPolicy(product = {}) {
  return cleanText(product.returnPolicy || 'For returns or exchanges, contact Scentivity as soon as possible after receiving your item.');
}

function productReviewsFor(product = {}) {
  const productName = cleanText(product.name || '').toLowerCase();
  const approved = (customerReviews || []).filter(review => review && review.approved !== false);
  if (!productName) return approved.slice(0, 3);
  const matched = approved.filter(review => cleanText(review.productsPurchased || review.product || '').toLowerCase().includes(productName));
  return (matched.length ? matched : approved).slice(0, 3);
}
...
function compactProductCard(product = {}, options = {}) {
  const name = cleanText(product.name || 'Untitled product');
  ...
  const brand = cleanText(product.brand || 'Scentivity');
  return `
    <article class="product-card compact-product-card ${available ? '' : 'is-unavailable'}" data-product-detail="${key}" tabindex="0" aria-label="View details for ${name}">
      <div class="compact-product-image">
        <img src="${image}" alt="${name}" loading="lazy" onerror="this.onerror=null;this.src='assets/scentivity-logo-fused.png';" />
        ${available ? `<span class="compact-product-badge">Available</span>` : `<span class="compact-product-badge coming">Coming soon</span>`}
      </div>
...
function renderHomepageShowcase() {
  ...
  homepageProductSlides.innerHTML = `
    <article class="showcase-slide active compact-showcase-slide" aria-label="${name}" data-product-detail="${key}" tabindex="0">
      <div class="showcase-image-wrap">
        <img src="${image}" alt="${name}" loading="lazy" onerror="this.onerror=null;this.src='assets/scentivity-logo-fused.png';" />
...
let activeProductDetailKey = '';
let activeProductDetailQty = 1;

function renderOtherProducts(currentKey = '') {
  const others = products
    .filter(product => product.available !== false && product._key !== currentKey)
    .slice(0, 4);
  if (!others.length) return '<p class="cart-small-note">More products will be added soon.</p>';
  return `
    <div class="related-products-grid">
      ${others.map(product => `
        <article class="related-product-card" data-product-detail="${product._key}" tabindex="0">
...
function renderProductDetail(product = {}) {
  ...
  activeProductDetailKey = key;
  activeProductDetailQty = Math.min(Math.max(1, activeProductDetailQty || 1), maxQty);
    ...
    <div class="product-detail-layout">
      ...
      <div class="product-detail-main">
        ...
        <div class="product-detail-rating-line">
          ...
          ${reviews ? `<a href="#productDetailReviews">${formatCompactCount(reviews)} reviews</a>` : '<em>No reviews yet</em>'}
          ${purchased ? `<em>${formatCompactCount(purchased)} bought</em>` : ''}
        </div>
        <strong class="product-detail-price">${price}</strong>
        ${stock ? `<p class="stock-note">${stock} available</p>` : `<p class="stock-note">${available ? 'Available' : 'Coming soon'}</p>`}
        ...
        <div class="product-detail-actions">
          <div class="detail-qty-control" aria-label="Quantity selector">
            ...
            <b id="productDetailQty">${activeProductDetailQty}</b>
            <button type="button" data-detail-qty="increase" aria-label="Increase quantity">+</button>
          </div>
          ${available
            ? `<button class="btn primary product-detail-add" type="button" data-product-detail-add="${key}">Add to Cart</button>`
        ...
        <section class="product-detail-section">
          <h3>Fragrance</h3>
          <p>${getProductFragranceNotes(product)}</p>
        </section>
        <section class="product-detail-section">
          <h3>Overview</h3>
          <p>${getProductDetailText(product)}</p>
        </section>
        <section class="product-detail-section">
          <h3>Ingredients</h3>
        ...
        <section class="product-detail-section">
          ...
          <p>${getProductReturnPolicy(product)}</p>
        </section>
      </div>
    </div>
    <div id="productDetailReviews">
      ${renderProductReviews(product)}
    </div>
    <section class="product-detail-section">
      <h3>Other available products</h3>
      ${renderOtherProducts(key)}
    </section>
  `;
}
...
function updateProductDetailQty(action) {
  const product = products.find(item => item._key === activeProductDetailKey);
  const maxQty = productAvailableQuantity(product || {}) || 99;
  if (action === 'increase') activeProductDetailQty = Math.min(maxQty, activeProductDetailQty + 1);
  if (action === 'decrease') activeProductDetailQty = Math.max(1, activeProductDetailQty - 1);
  const qtyTarget = document.querySelector('#productDetailQty');
  if (qtyTarget) qtyTarget.textContent = String(activeProductDetailQty);
}
...
function addToCart(productKey, quantity = 1) {
  ...
  const allowedQuantity = stock > 0 ? Math.max(0, Math.min(addQuantity, stock - currentQty)) : addQuantity;
  ...
  if (existing) {
    ...
    cart.push({ ...productSnapshot(product), quantity: allowedQuantity });
  }
  saveCart();
  renderCart();
  openCart();
...
function updateCartItem(key, action) {
  ...
  if (action === 'increase') {
    if (stock > 0 && Number(item.quantity || 0) >= stock) {
      alert(`Only ${stock} of this product is currently available.`);
      return;
    }
    item.quantity += 1;
