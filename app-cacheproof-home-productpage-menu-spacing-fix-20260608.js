// SCENTIVITY_HOME_PRODUCTPAGE_MENU_SPACING_FIX_20260608
...
// Final homepage spacing/menu/click fixes
(function scentivityHomeMenuSpacingFixes() {
  function productPageUrlSafe(productOrKey = '') {
    if (typeof productPageUrl === 'function') return productPageUrl(productOrKey);
    if (productOrKey && typeof productOrKey === 'object') {
      const key = cleanText(productOrKey._key || productOrKey.id || productOrKey.slug || '');
    ...
    return `product.html?product=${encodeURIComponent(cleanText(productOrKey || ''))}`;
  }

  document.addEventListener('click', event => {
    // Customer review section must never open cart/checkout.
    if (event.target.closest('#testimonials')) {
    ...
    // Product card click: works on both left and right cards in two-column mobile grid.
    const card = event.target.closest('#productGrid .product-card, #homepageProductSlides .product-click-card, #dealOfWeekCard.product-click-card, .product-view-button[data-product-key]');
    if (!card) return;

    if (event.target.closest('.add-to-cart, [data-deal-product-key], .compact-cart-button, button.add-combo-to-cart, a, input, select, textarea, label')) {
      return;
    }

    const key = card.dataset.productKey || card.querySelector('[data-product-key]')?.dataset.productKey;
    if (!key) return;

    const product = Array.isArray(products) ? products.find(item => item._key === key || item.id === key || item.slug === key) : null;
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
    window.location.href = productPageUrlSafe(product || key);
  }, true);
  ...
  document.addEventListener('DOMContentLoaded', openCartFromUrl);
  window.addEventListener('load', openCartFromUrl);
})();
