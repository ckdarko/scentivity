// SCENTIVITY_HOMEPAGE_PRODUCTS_RESTORE_20260608
// SCENTIVITY_PRODUCT_PAGE_CLICK_FIX_20260608
...
function compactProductCard(product = {}, options = {}) {
  ...
  return `
    <article class="product-card compact-product-card product-link-card ${inStock ? '' : 'is-unavailable'}" data-product-url="${productPageUrl(key)}" tabindex="0" aria-label="View details for ${name}">
      <div class="compact-product-image">
        ...
        <span class="compact-product-badge ${inStock ? '' : 'coming'}">${inStock ? 'Available' : 'Out of stock'}</span>
      </div>
...
function getVisibleProducts() {
  ...
  return products.filter(product => {
    ...
    const matchesMain = activeMainCategory === 'all' || mainCategory === activeMainCategory;
...
function getHomepageSlides() {
  return products
    ...
    .map(product => ({
      ...
      _slideType: product.available === false ? 'coming' : 'available'
    }));
}
...
function renderHomepageShowcase() {
  ...
  homepageProductSlides.innerHTML = `
    <article class="showcase-slide active compact-showcase-slide product-link-card" aria-label="${name}" data-product-url="${productPageUrl(key)}" tabindex="0">
      <div class="showcase-image-wrap">
        <img src="${image}" alt="${name}" loading="lazy" onerror="this.onerror=null;this.src='assets/scentivity-logo-fused.png';" />
        <span class="showcase-badge ${inStock ? 'available' : 'soon'}">${inStock ? 'Available now' : 'Out of stock'}</span>
      </div>
      <div class="showcase-copy">
        <p class="eyebrow">${inStock ? 'Available now' : 'Out of stock'} • ${showcaseIndex + 1} of ${slides.length}</p>
        <h3>${name}</h3>
...
function openProductDetail(productKey) {
  ...
  window.location.href = productPageUrl(productKey);
}
...
// Product page navigation fix: cards open product.html, not contact/catalogue/feedback/cart.
